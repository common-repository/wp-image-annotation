...
function getResults() {
	...
	foreach ($result as $topten) {		
		if( (get_option('wp_image_annotation_comments') == '0') ) {
			...
			if(get_option('wp_image_annotation_gravatar_deafult') != '') {
				$defaultgravatar = get_bloginfo('template_url').get_option('wp_image_annotation_gravatar_deafult');
			} else {
				$defaultgravatar = '';
			}
			...
			if($commentApprove == 1) {
				...
				if( (get_option('wp_image_annotation_gravatar') == '0') ) {
