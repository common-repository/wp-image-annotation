<?php 
/*
Plugin Name: Wp Image Annotations
Plugin URI: http://www.superwhite.cc/wp/image-annotation-plugin
...
function load_image_annotation_js() {
	...
	<?php if( (get_option('wp_image_annotation_display') == '0' && $plugin != 0) ) { ?>
		jQuery.noConflict();
		jQuery(document).ready(function(){
				//image annotaion
				jQuery("<?php echo get_option('wp_image_annotation_postcontainer'); ?> img").each(function() {						
						...
						if(idname.substring(4,idname.length) != 'exclude') {
							...
							<?php if (get_option('wp_image_annotation_admin') == '0') { ?>
							addablecon = false;
							<?php } else { ?>
							...
							<?php if( (get_option('wp_image_annotation_autoimageid') == '0') ) { ?>
							...
							if(imgid.substring(0,4) == "img-") {
								...
								<?php if( (get_option('wp_image_annotation_dia_imgtag') == '0') ) { ?>
								...
								var imagenotedesc = "<?php echo get_option('wp_image_annotation_mouseoverdesc'); ?>";
								var imagelinkdesc = "<?php echo get_option('wp_image_annotation_linkdesc'); ?>";
...
if( (get_option('wp_image_annotation_display') == '0') ) {
	if( (get_option('wp_image_annotation_thumbnail') == '0') ) {
...
//*************** Admin function ***************
function wpimageannotation_admin() {
	include('imageannotation-admin.php');
}

function wpimageannotation_admin_actions() {
	add_menu_page('wp-Image-Annotation', 'wp-Image-Annotation', 'manage_options', 'wp-Image-Annotation', 'wpimageannotation_admin', plugins_url('icon.png',__FILE__));
	changeTableName();
}
...
if (is_admin())
{
	add_action('admin_menu', 'wpimageannotation_admin_actions');
}
?>